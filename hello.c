...
void count_sort (int arr[],int size){
	...
	// this property of maintaining the order this sorting is also called stable sort
	for (i = size-1; i >= 1; i--) {
...
void insert_sort (int arr[],int size) {
	...
	for (i = 1; i < size; i++) {
		...
		for (j = i; j >= 0; j--){
			...
			// is the most important aspect of this algo and target is to fit tmp in this right position in (0,j)
...
// merge sort implements a divide and conqure algo its divides up the array
//in small sub arrays (size 2) up to when no more division can happen and the start merging those divided subarrays
// but during merge each elemnt of the array is compared and stored in a sorted order in the merged array
