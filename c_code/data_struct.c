...
/*
 *  walk the list when encounter val < x create a new node containing val
 *  and add it to the head. now in list using the pos pinter delete the old node
 *  containing the val, update the prev/pos pinter according and continue to walk
 *  the list and repeat.
 * */
void
...
int list_palindrome(List **head) {
	...
	start = *head;
	dbl = start->next;
    // one item list so this should be a palindrom
	if(!dbl) {
		return 1;
	}
